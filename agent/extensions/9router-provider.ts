...
function estimateContextWindow(modelId: string): number {
	const id = modelId.toLowerCase();
	if (id.includes("claude") || id.includes("sonnet") || id.includes("haiku") || id.includes("opus")) {
		return 200000;
	}
...
export default async function (pi: ExtensionAPI) {
	const baseUrl =
		process.env.PI_9ROUTER_BASE_URL?.replace(/\/+$/, "") || "http://localhost:20128/v1";
	...
	try {
		...
		console.warn(`[9router] Using fallback model list (${FALLBACK_MODELS.length} models)`);
