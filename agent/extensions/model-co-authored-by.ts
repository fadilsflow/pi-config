...
export default function modelCoAuthoredBy(pi: ExtensionAPI) {
	pi.on("tool_call", async (event, ctx) => {
		...
		event.input.command = appendCoAuthorTrailer(event.input.command, coAuthor);
	});
}

function getCoAuthor(model: { provider?: string; id?: string; name?: string } | undefined): CoAuthor | undefined {
	if (!model) return undefined;

	const modelKey = [model.provider, model.id, model.name].filter(Boolean).join(" ");
...
function appendCoAuthorTrailer(cmd: string, coAuthor: CoAuthor): string {
	...
	const separatorPadding = afterCommit.startsWith("&&") || afterCommit.startsWith("||") ? " " : "";

	return `${cmd.slice(0, segment.end).trimEnd()}${trailerArgs}${separatorPadding}${afterCommit}`;
}

function findCommitSegment(cmd: string): { start: number; end: number } | undefined {
...
function hasMessageFlag(segment: string): boolean {
	return /(?:^|\s)-[^\s-]*m\b/.test(segment) || /(?:^|\s)--message(?:\s|=|$)/.test(segment);
}

function hasCoAuthorTrailer(commitCommand: string, coAuthor: CoAuthor): boolean {
	const normalized = commitCommand.toLowerCase();
	return normalized.includes("co-authored-by:") || normalized.includes(coAuthor.email.toLowerCase());
}
